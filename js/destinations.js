...
window.DestinationEngine = {
    ...
    async load() {
        ...
        let destinations = [];
        const local = await this.fetchJson("data/destinations.json?engine=5", {}, 8000);
        ...
        const extras = await Promise.all(extraFiles.map(file => this.fetchJson(file + "?engine=5", {}, 6000)));
    ...
    normalise(destinations) {
        destinations.forEach(item => {
            ...
            if (id === "alibaug" || id === "alibag" || name === "alibaug" || name === "alibag" || name === "alibagh") {
                ...
                item.imageSearchTerms = ["Varsoli Beach Alibaug Maharashtra", "Alibaug Beach Maharashtra", "Kolaba Fort Alibaug Maharashtra"];
                item.tags = [...new Set([...item.tags, "beach", "coastal", "fort", "heritage"])]
            }
            if (id === "jalgaon" || name === "jalgaon") {
                item.image = "";
                item.imageSearchTerms = ["Jalgaon Maharashtra", "Jalgaon city Maharashtra", "Jalgaon district Maharashtra"];
            }
        });
    },
    ...
    async find(searchText) {
        ...
        const grouped = this.destinations.find(item => String(item.country || "").trim().toLowerCase() === search || String(item.continent || "").trim().toLowerCase() === search || String(item.state || item.region || "").trim().toLowerCase() === search);
    ...
    async search(searchText) {
        ...
        const results = this.destinations.filter(item => {
            ...
            const haystack = [item.name, item.id, item.country, item.continent, item.region, item.state, ...(Array.isArray(item.tags) ? item.tags : []), ...aliases].filter(Boolean).join(" ").toLowerCase();
...
// Domestic search safety: if the user types while the catalogue is still loading,
// the old page had no input handler yet. Wait for the catalogue, then replay the
// input event so the page's normal renderer handles it (including images/cart).
(function installDomesticSearchSafetyLayer() {
    const boot = () => {
        const input = document.getElementById("india-search");
        if (!input || input.dataset.safetySearchInstalled === "1") return;
        input.dataset.safetySearchInstalled = "1";
        input.addEventListener("input", async event => {
            if (event.__natureToursRetry) return;
            if (window.DestinationEngine.destinations.length) return;
            try {
                await window.DestinationEngine.load();
                const retry = new Event("input", { bubbles: true });
                retry.__natureToursRetry = true;
                input.dispatchEvent(retry);
            } catch (_) {}
        }, true);
    };
