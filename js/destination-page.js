...
(function () {
    ...
    async function loadDestinations() {
        ...
        try {
            const response = await fetch("data/destinations.json?fallback=place-images-3");
    ...
    function imageQueries(destination) {
        ...
        const queries = Array.isArray(destination.imageSearchTerms) ? [...destination.imageSearchTerms] : [`${name}, ${region}`, `${name} tourist attraction`, `${name} India`];
        if (tags.includes("beaches") || tags.includes("beach") || tags.includes("coastal")) queries.unshift(`${name} beach India`);
        if (tags.includes("fort") || tags.includes("heritage") || tags.includes("history")) queries.unshift(`${name} fort India`);
        if (tags.includes("wildlife") || tags.includes("national-park")) queries.unshift(`${name} national park India`);
        if (tags.includes("mountains") || tags.includes("himalaya")) queries.unshift(`${name} mountains India`);
    ...
    async function init() {
        ...
        if (!destination) { container.innerHTML = `<div class="glance-card"><h2>Destination not found</h2><p>We could not find “${esc(requested)}” in the destination catalogue.</p><p><a href="domestic.html">← Back to Domestic Explorer</a></p></div>`; return; }
